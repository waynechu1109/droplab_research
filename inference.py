...
try:
    ...
    state_dict = torch.load("ckpt/sdf_model_5000_weight:(20,1,0.5).pt", map_location=device, weights_only=True)
...
points = np.stack([grid_x, grid_y, grid_z], axis=-1).reshape(-1, 3)  # [M, 3] = [128*128*128, 3]
...
# 建立 mesh（trimesh 替代 Open3D）
mesh = trimesh.Trimesh(vertices=verts, faces=faces, process=False)
mesh.export("output/sdf_surface_5000_weight:(20,1,0.5).ply")
